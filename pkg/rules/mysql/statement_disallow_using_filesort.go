...
func (a *StatementDisallowUsingFilesortAdvisor) Check(ctx context.Context, statements string, rule *types.SQLReviewRule, checkContext advisor.SQLReviewCheckContext) ([]*types.Advice, error) {
	...
	// Create the rule
	statementDisallowUsingFilesortRule := NewStatementDisallowUsingFilesortRule(types.SQLReviewRuleLevel(level), string(rule.Type))
