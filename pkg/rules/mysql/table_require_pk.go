...
// columnSet represents a set of column names
type columnSet map[string]bool

// newColumnSet creates a new column set from a list of column names
...
// TableRequirePKRule is the ANTLR-based implementation for checking table requires primary key
...
// NewTableRequirePKRule creates a new ANTLR-based table require PK rule
func NewTableRequirePKRule(level types.SQLReviewRuleLevel, title string, catalog *catalog.Finder) *TableRequirePKRule {
...
func (r *TableRequirePKRule) dropColumn(tableName string, columnName string) bool {
	if _, ok := r.tables[tableName]; !ok {
		if r.catalog != nil {
			// Look for primary key index in the catalog like Bytebase does
			primaryKey := r.catalog.Origin.FindPrimaryKey(&catalog.PrimaryKeyFind{
				SchemaName: "", // MySQL default schema
				TableName:  tableName,
			})
			if primaryKey != nil {
				r.tables[tableName] = newColumnSet(primaryKey.ExpressionList())
			} else {
				r.initEmptyTable(tableName)
			}
		} else {
...
// TableRequirePKAdvisor is the advisor using ANTLR parser for table require PK checking
...
func (a *TableRequirePKAdvisor) Check(ctx context.Context, statements string, rule *types.SQLReviewRule, checkContext advisor.SQLReviewCheckContext) ([]*types.Advice, error) {
	...
	// Create the rule
	tableRule := NewTableRequirePKRule(types.SQLReviewRuleLevel(level), string(rule.Type), catalogFinder)
