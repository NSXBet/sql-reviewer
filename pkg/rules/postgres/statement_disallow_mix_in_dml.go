...
func init() {
	advisor.Register(types.Engine_POSTGRES, advisor.Type(advisor.SchemaRuleStatementDisallowMixInDML), &StatementDisallowMixInDMLAdvisor{})
}
...
func (c *statementDisallowMixInDMLChecker) addDDLAdvice(ctx antlr.ParserRuleContext, _ string) {
	...
	c.adviceList = append(c.adviceList, &types.Advice{
		Status:  c.level,
		Code:    int32(advisor.PostgreSQLStatementDisallowMixInDML),
		Title:   c.title,
