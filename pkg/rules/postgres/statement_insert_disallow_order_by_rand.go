...
func (*StatementInsertDisallowOrderByRandAdvisor) Check(ctx context.Context, checkCtx advisor.Context) ([]*types.Advice, error) {
	...
	checker := &statementInsertDisallowOrderByRandChecker{
		level:          level,
...
type statementInsertDisallowOrderByRandChecker struct {
...
func (c *statementInsertDisallowOrderByRandChecker) EnterInsertstmt(ctx *parser.InsertstmtContext) {
...
func (*statementInsertDisallowOrderByRandChecker) hasOrderByRandom(selectCtx parser.ISelectstmtContext) bool {
	if selectCtx == nil {
		return false
	}
