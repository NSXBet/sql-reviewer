...
func (d *DatabaseState) pgAntlrWalkThrough(tree any) error {
	// Handle pgparser.ParseResult wrapper
	var root parser.IRootContext
	switch t := tree.(type) {
	case parser.IRootContext:
		root = t
	case *pgparser.ParseResult:
		// Handle ParseResult from pgparser - extract Tree field
		if treeNode, ok := t.Tree.(parser.IRootContext); ok {
			root = treeNode
		} else {
			return errors.Errorf("parse result tree is not IRootContext: %T", t.Tree)
		}
	default:
...
func (l *pgAntlrCatalogListener) processAlterTableCmd(
	schema *SchemaState,
	table *TableState,
	cmd parser.IAlter_table_cmdContext,
) {
...
func (l *pgAntlrCatalogListener) alterTableDropColumn(schema *SchemaState, table *TableState, columnName string, ifExists bool) {
	...
	if len(viewList) > 0 {
		l.setError(&WalkThroughError{
			...
			Content: fmt.Sprintf(
				"Cannot drop column %q in table %q.%q, it's referenced by view: %s",
				column.name,
				schema.name,
				table.name,
...
func (l *pgAntlrCatalogListener) alterTableAlterColumnType(
	schema *SchemaState,
	table *TableState,
	columnName string,
	typeString string,
	...
	if len(viewList) > 0 {
		l.setError(&WalkThroughError{
			Type: ErrorTypeColumnIsReferencedByView,
			Content: fmt.Sprintf(
				"Cannot alter type of column %q in table %q.%q, it's referenced by view: %s",
				column.name,
				schema.name,
				table.name,
				strings.Join(viewList, ", "),
...
func (l *pgAntlrCatalogListener) alterTableAddColumn(
	schema *SchemaState,
	table *TableState,
...
func (l *pgAntlrCatalogListener) alterTableAddConstraint(
	schema *SchemaState,
	table *TableState,
...
func (l *pgAntlrCatalogListener) alterTableDropConstraint(
	schema *SchemaState,
	table *TableState,
...
func (*pgAntlrCatalogListener) renameConstraint(
	schema *SchemaState,
	table *TableState,
	oldName string,
	newName string,
...
func (l *pgAntlrCatalogListener) dropTable(anyName parser.IAny_nameContext, ifExists bool) *WalkThroughError {
	...
	if len(viewList) > 0 {
		return &WalkThroughError{
			...
			Content: fmt.Sprintf(
				"Cannot drop table %q.%q, it's referenced by view: %s",
				schema.name,
				table.name,
				strings.Join(viewList, ", "),
			),
...
// parseViewName parses a quoted view name in the format "schema"."view".
...
// existedViewList returns the list of existing views from a dependency map.
...
// getColumn gets a column by name with proper error handling.
