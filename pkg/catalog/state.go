...
func newDatabaseState(d *types.DatabaseSchemaMetadata, context *FinderContext) *DatabaseState {
	...
	for _, schema := range d.Schemas {
		for _, view := range schema.Views {
			for _, dependencyColumn := range view.DependentColumns {
...
func newSchemaState(s *types.SchemaMetadata, context *FinderContext) *SchemaState {
	schema := &SchemaState{
		ctx:           context.Copy(),
		name:          s.Name,
...
func newViewState(v *types.ViewMetadata) *ViewState {
	return &ViewState{
		name:       v.Name,
...
func newTableState(t *types.TableMetadata, context *FinderContext) *TableState {
	table := &TableState{
		name:           t.Name,
	...
	for i, column := range t.Columns {
		...
		case types.Engine_MYSQL, types.Engine_TIDB, types.Engine_MARIADB, types.Engine_OCEANBASE:
...
func newColumnState(c *types.ColumnMetadata, position int) *ColumnState {
	defaultValue := (*string)(nil)
	if c.HasDefault && c.DefaultString != "" {
		defaultValue = copyStringPointer(&c.DefaultString)
	}
	return &ColumnState{
		name:           c.Name,
...
func newIndexState(i *types.IndexMetadata) *IndexState {
	index := &IndexState{
		name:           i.Name,
...
func (d *DatabaseState) FindIndex(find *IndexFind) (string, *IndexState) {
	...
	needMatchTable := (d.dbType != types.Engine_POSTGRES || find.SchemaName == "" || find.TableName != "")
	if needMatchTable {
...
// getTable finds the table by name (case sensitive or insensitive based on context).
func (s *SchemaState) getTable(tableName string) (*TableState, bool) {
...
// compareIdentifier compares two identifiers with case sensitivity handling.
